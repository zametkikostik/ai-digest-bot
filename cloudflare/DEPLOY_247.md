...
| Ресурс | Лимит | Статус бота |
|--------|-------|-------------|
| **Запросы/день** | 100,000 | ✅ Хватит на ~2000 пользователей |
| **CPU время/запрос** | 10 ms | ⚠️ API запросы через `waitUntil` |
| **Wall time/запрос** | 15 секунд | ✅ Достаточно |
| **Размер скрипта** | 1 MB | ✅ Скрипт ~15 KB |
| **KV записи/день** | 100,000 | ✅ Кэш котировок (5 мин) |
| **KV чтения/день** | 100,000 | ✅ |
| **D1 чтения/день** | 5,000 | ⚠️ Ограничено |
| **D1 записи/день** | 1,000 | ⚠️ Ограничено |

### ⚙️ Оптимизация

**Кэширование котировок:**
- Криптовалюты: 5 минут кэш
- Акции MOEX: 5 минут кэш
- Экономия API запросов: ~12x

**Пример:**
- Без кэша: 1000 пользователей × 2 запроса = 2000 API calls/день
- С кэшем (5 мин): 288 запросов/день (экономия 85%)
