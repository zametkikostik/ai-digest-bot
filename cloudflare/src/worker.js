...
function getSchoolKB(lang) { const kb=[],row=[]; SCHOOL.forEach((s,i)=>{row.push({text:s,callback_data:"school_"+s});if(row.length===2||i===SCHOOL.length-1){kb.push(row);row=[];}});kb.push([{text:lang==="ru"?"🔙 Назад":"🔙 Back",callback_data:"back_main"}]);return {inline_keyboard:kb}; }
function getUniKB(lang) { const kb=[],row=[]; UNI.forEach((s,i)=>{row.push({text:s,callback_data:"uni_"+s});if(row.length===2||i===UNI.length-1){kb.push(row);row=[];}});kb.push([{text:lang==="ru"?"🔙 Назад":"🔙 Back",callback_data:"back_main"}]);return {inline_keyboard:kb}; }
