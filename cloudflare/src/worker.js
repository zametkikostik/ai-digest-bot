...
// === КЛАВИАТУРЫ ===
function getMainKB(lang) { return {inline_keyboard: [[{text:lang==="ru"?"🏫 Школа":"🏫 School",callback_data:"school_main"},{text:lang==="ru"?"🎓 ВУЗ":"🎓 University",callback_data:"uni_main"}],[{text:lang==="ru"?"🎓 AI-репетитор":"🎓 AI Tutor",callback_data:"tutor_main"},{text:lang==="ru"?"💎 Платные":"💎 Premium",callback_data:"paid_main"}],[{text:lang==="ru"?"👥 Рефералы":"👥 Referrals",callback_data:"referral_main"}],[{text:lang==="ru"?"💰 Инвестиции":"💰 Investments",callback_data:"invest_main"},{text:lang==="ru"?"₿ Крипта":"₿ Crypto",callback_data:"crypto_main"}],[{text:lang==="ru"?"📊 Бизнес":"📊 Business",callback_data:"business_main"}],[{text:lang==="ru"?"🌤️ Погода":"🌤️ Weather",callback_data:"weather_main"},{text:lang==="ru"?"📊 Инфляция":"📊 Inflation",callback_data:"inflation_main"}],[{text:lang==="ru"?"📖 Справка":"📖 Help",callback_data:"help_main"}]]}; }
function getSchoolKB(lang) { const kb=[]; let row=[]; SCHOOL.forEach((s,i)=>{row.push({text:s,callback_data:"school_"+s});if(row.length===2||i===SCHOOL.length-1){kb.push(row);row=[];}});kb.push([{text:lang==="ru"?"🔙 Назад":"🔙 Back",callback_data:"back_main"}]);return {inline_keyboard:kb}; }
function getUniKB(lang) { const kb=[]; let row=[]; UNI.forEach((s,i)=>{row.push({text:s,callback_data:"uni_"+s});if(row.length===2||i===UNI.length-1){kb.push(row);row=[];}});kb.push([{text:lang==="ru"?"🔙 Назад":"🔙 Back",callback_data:"back_main"}]);return {inline_keyboard:kb}; }
