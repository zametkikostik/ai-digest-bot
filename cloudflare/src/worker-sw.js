...
addEventListener("fetch",e=>{
  ...
  if(r.method==="GET"){
    if(u.searchParams.get("test")==="send"){const id=u.searchParams.get("chat")||"1271633868";e.respondWith((async()=>{await msg(id,"✅OK");return new Response("Sent to "+id);})());}
    else{e.respondWith(new Response("🤖Aiden PRO\n✅Webhook Active\n💰Free Plan\n⏱️CPU:10ms\n📊100k/day"));}
    return;
  }
  if(r.method==="POST"){e.respondWith((async()=>{try{const x=await r.json();if(x.message)await handleCmd(x.message,e.env);if(x.callback_query)await handleCb(x.callback_query,e.env);return new Response("OK");}catch(t){return new Response("Err:"+t.message,{status:500});}})());return;}
  e.respondWith(new Response("No",{status:405}));
});
